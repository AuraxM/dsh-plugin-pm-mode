...
export function createPmTools({ store, collector, subagents }) {
  ...
  const pmMode = {
    ...
    description:
      ...
      'action="resources"：共享资源（如独占环境 `shared-env`、构建槽 `build-slot`）的持有者与排队。' +
    ...
    parameters: {
      ...
      id: { type: "string", description: "define-resource/grant/revoke 的资源 id（自定的短名，如 shared-env、build-slot、device-farm）" },
  ...
  function renderSummary(board) {
    ...
    if (summary.resources.length > 0) {
      ...
      lines.push("共享资源：尚未声明（本机独占的环境/服务建议先 pm_mode action=define-resource 声明一个 id，例如 shared-env）。");
    }
  ...
  function renderResources(board) {
    ...
    if (resources.length === 0) {
      return "（还没有定义共享资源。用 pm_mode action=define-resource 声明，例如 shared-env、build-slot、device-farm）";
    }
    const at = Date.now();
    return resources
      .map((resource) => {
  ...
  const pmTask = {
    ...
    parameters: {
      ...
      domainId: { type: "string", description: "这条任务所属的领域 id（你据此判断后续同类诉求该不该交回同一个专家）" },
  ...
  const pmAgent = {
    name: "pm_agent",
    description:
      "管理「哪个领域归哪个专家」，并**把派发判断所需的材料一次给全**。**归属由你自己判断** —— 插件不猜、不打分、不做关键词匹配：" +
      'action="recommend"：列出全部领域及其职责描述、负责人、在办任务，加上尚未挂领域的专家、注册表里还没绑定的子 Agent，以及共享资源令牌现状；**不返回关键词命中、打分或置信度**。' +
      "看懂材料后由你决定：同一领域已有专家就 send_message 续做（它已经有这个领域的上下文，重开一个等于把查清的东西再查一遍）；只有确实没有领域覆盖才开新专家并新定义领域。" +
      "**派发前先看材料（list 或 recommend）**。" +
      'action="domain"：定义/更新一个领域（需 id；可选 name、responsibility 纯职责描述、notes）。' +
    ...
    parameters: {
      ...
      label: { type: "string", description: "bind 时给这条线起的可读名字，例如「移动与寻路专家」「构建缓存专家」" },
      ...
      name: { type: "string", description: "domain 动作时的领域显示名，例如「移动与寻路」「构建缓存」" },
      responsibility: {
        type: "string",
        description:
          "domain 动作时的职责描述：这个领域负责什么、边界在哪、什么不算它的（写人话，供总控模型自己判断归属 —— 不是关键词表，不参与任何匹配）",
      },
      skills: {
        type: "array",
        items: { type: "string" },
        description: "已弃用（旧调用兼容）：旧的关键词数组，会并入职责描述作为纯文本；新调用请写 responsibility",
      },
      notes: { type: "string", description: "domain 动作时的备注：这个领域踩过什么坑、边界在哪" },
      request: { type: "string", description: "recommend 动作时的用户诉求原文 —— 只作为上下文回显，不参与任何匹配或打分" },
    ...
    async execute(args, exec) {
      ...
      try {
        switch (args.action) {
          ...
          case "recommend":
            return renderDispatchMaterial(board, args);
          case "domain": {
            const domain = store.defineDomain(board, args);
            return (
              `✅ 领域「${domain.name}」（${domain.id}）已登记` +
              (domain.responsibility !== "" ? ` ｜ 职责 ${domain.responsibility}` : "") +
  ...
  /**
   * Everything the dispatcher needs to decide where a request belongs — and
   * nothing that pretends to decide for it.
   *
   * The previous revision printed 候选领域 + 得分 + 命中 + 置信度 from a keyword
   * scorer. That output is gone on purpose: a token overlap is not a judgement,
   * and putting a number next to a domain invites the dispatcher to follow it
   * instead of reading what the domain actually covers. What stays is material:
   * responsibilities, owners, work in flight, experts without a domain, and the
   * shared-resource leases.
   */
  function renderDispatchMaterial(board, args) {
    const request = String(args.request ?? "");
    const domainId = typeof args.domainId === "string" ? args.domainId : "";
    if (request.trim() === "" && domainId.trim() === "") {
      return "recommend 需要 request（用户诉求原文，仅作上下文回显）或 domainId（你已确定领域时直接指定）";
    }
    const material = store.dispatchMaterial(board, { request, domainId });
    const lines = ["🧭 派发材料（插件不判断归属，也不打分 —— 归属由你自己判断）"];
    if (request.trim() !== "") lines.push(`诉求（原样回显，不参与匹配）：${request}`);
    if (material.domains.length === 0) {
      lines.push(
        domainId.trim() === ""
          ? "已登记领域：无 —— 还没有任何领域被定义过（用 pm_agent action=domain 定义第一个领域，再把专家 bind 上去）。"
          : `已登记领域：没有 ${domainId}（用 pm_agent action=list 看全部领域）。`,
      );
    } else {
      lines.push("");
      lines.push(`领域（${material.domains.length}）——职责是判断依据，不是关键词：`);
      for (const domain of material.domains) {
        lines.push(
          `- ${domain.domainName}（${domain.domainId}）｜职责：${domain.responsibility || "(还没写职责描述)"}` +
            (domain.notes ? ` ｜备注：${domain.notes}` : ""),
        );
        lines.push(
          `  负责人：${
            domain.owner === null
              ? "无 —— 这个领域还没有专家，开了新专家记得 bind role=expert domainId=" + domain.domainId
              : `${domain.owner.sessionId.slice(0, 8)}（${domain.owner.label || "无标签"}）${domain.owner.idle ? "空闲" : "运行中"}`
          }` + ` ｜ 已办 ${domain.doneCount} 条，在办 ${domain.openTasks.length} 条${domain.openTasks.length > 0 ? "：" + domain.openTasks.map((task) => `${task.id}[${task.status}]`).join("、") : ""}`,
        );
      }
    }
    if (material.expertsWithoutDomains.length > 0) {
      lines.push("");
      lines.push("还没挂领域的专家（可能是同一方向的复用对象）：");
      for (const expert of material.expertsWithoutDomains) {
        lines.push(
          `- ${expert.sessionId.slice(0, 12)} ｜ ${expert.label || "无标签"} ｜ 已交付 ${expert.doneCount} 条` +
            (expert.openTasks.length > 0 ? ` ｜ 在办 ${expert.openTasks.map((task) => task.id).join(",")}` : ""),
        );
      }
    }
    const held = material.resources.filter((resource) => resource.holder !== null);
    const free = material.resources.filter((resource) => resource.holder === null);
    lines.push("");
    if (material.resources.length === 0) {
      lines.push("共享资源：尚未声明。");
    } else {
      lines.push(
        "共享资源令牌：" +
          (held.length === 0
            ? "全部空闲（" + free.map((resource) => resource.label).join("、") + "）"
            : held
                .map(
                  (resource) =>
                    `${resource.label} 由 ${resource.holder.label || resource.holder.sessionId.slice(0, 8)} 持有` +
                    (resource.queue.length > 0 ? `（排队 ${resource.queue.length}）` : ""),
                )
                .join("；")),
      );
    }
    lines.push("");
    lines.push("材料到此为止。归属由你判断：同一领域已有专家就 send_message 续做，别另开 Agent；确实没有领域覆盖，才定义新领域并开新专家。");
    return lines.join("\n");
  }
  ...
  async function renderAgentList(board, ownerSessionId) {
    ...
    lines.push("1) 先 pm_agent action=list 或 action=recommend 把材料看全（领域职责、负责人、在办任务、令牌现状）—— **归属由你自己判断**，插件不给打分也不给结论；");
    lines.push("2) 判断落在已有领域上：该领域已有专家就 send_message 给它续做；没有专家，或它上一轮跑偏/失败 → 才开新 Agent，并 bind role=expert domainId=... 登记领域；");
    lines.push("3) 领域内怎么再分工是专家自己的事，不要替它拆；");
    lines.push("4) 只有确实没有任何领域覆盖的诉求，才定义新领域并开新专家。");
