 ...
 *
 * Colors go through the product's own `--dsw-*` custom properties (see
 * `lib/client/styles.js`), so the panel follows the light/dark theme instead
 * of pinning one palette.
 *
 ...
 * swatches only — a saturated block reads the same in both themes, but ink on
 * a page-like surface must follow the theme. Text and chips therefore use the
 * theme's own tokens (see `lib/client/styles.js`), never these values: a
 * hard-coded #e3b341 as a label color is 1.9:1 on the light theme's white.
 */
export const COLORS = {
  planned: "#6b7280",
  ready: "#3fb950",
 ...
 *
 * The chip itself is a tinted surface with readable text; its ACCENT lives in
 * the `--pmb-accent` custom property, so one class per status covers both
 * themes. Filling the chip with the status color and writing white on top (the
 * previous revision) is 1.9:1 for 阻塞 / 2.2:1 for 完成 in the light theme.
 *
 * An unknown status falls back to `s-planned` for the same reason the color
 * map did: a new status must render, not crash.
 */
