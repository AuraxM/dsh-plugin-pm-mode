 ...
 *   - resources:   exclusive leases over shared singletons (a long-running
 *                  service, an editor/lock-holding tool, a build slot, ...) plus
 *                  their wait queue. The id is whatever the dispatcher declares
 *                  — see `defineResource` for why the plugin ships no default;
...
/**
 * What a domain is FOR, as the one line the dispatcher reads before deciding.
 *
 * Deliberately PROSE, not a token list. The previous revision matched a request
 * against `skills` keywords with a bigram scorer and printed 得分/命中/置信度;
 * that put a lexical accident where a judgement belongs, and it pushed the
 * operator into writing keyword soup ("写全一点") so the matcher could work.
 * The dispatcher model reads the board itself now, so what it needs is the
 * domain's responsibility in sentences — who it covers, and where its edge is.
 *
 * `skills` (the retired keyword array) is still folded in for boards written
 * before this change: their old keywords become plain text in the same line, so
 * nothing has to be migrated and no board reads as empty.
 */
export function domainResponsibility(domain) {
  const explicit = str(domain?.responsibility).trim();
  if (explicit !== "") return explicit;
  const legacy = strArray(domain?.skills).map((item) => item.trim()).filter((item) => item !== "");
 ...
 * in it — the whole reason a follow-up request is sent back to the same agent
 * instead of to a fresh one. `responsibility` is the prose the dispatcher
 * judges by; `skills` is the retired keyword array, kept only so a board
...
function normalizeDomain(id, domain) {
  return {
    ...
    responsibility: str(domain.responsibility) === "" ? domainResponsibility(domain) : str(domain.responsibility),
...
export class BoardStore {
  ...
  defineDomain(board, input) {
    const id = str(input.id).trim();
    if (id === "") throw new Error("领域需要 id（如 move-and-path、build-cache）");
    const at = nowMs();
    const existing = board.domains[id];
    // `responsibility` is the field to write. `skills` is still accepted (and
    // still stored) because prompts and boards from before the change carry it;
    // `domainResponsibility` folds it into the prose instead of erroring.
    ...
    if (existing === undefined) {
      this.record(board, { kind: "domain-defined", label: domain.name, detail: domain.responsibility });
    }
    return domain;
  }
  ...
  /**
   * One plain snapshot of everything a dispatcher needs to decide where a
   * request belongs.
   *
   * There is deliberately NO matching, ranking or scoring here. The previous
   * revision tokenised the request, overlapped it with each domain's `skills`
   * keywords and printed 得分 / 命中 / 置信度 — a lexical accident dressed as a
   * judgement, which also made the operator write keyword soup so the matcher
   * could fire. The dispatcher model reads this snapshot and decides; the board
   * only records what it decided (`pm_agent bind`, `pm_task create domainId`).
   *
   * The snapshot lists BOTH sides of the decision:
   *   - every domain, its prose responsibility, its owner and its work;
   *   - the experts that own no domain yet, and the registered subagents the
   *     board has not bound to a task (reuse candidates that a keyword matcher
   *     could never surface);
   *   - the shared-resource leases (who holds what, who is queued).
   */
  dispatchMaterial(board, input = {}) {
    ...
    for (const id of board.domainOrder) {
      ...
      if (wantedId !== "" && wantedId !== id) continue;
      const tasks = this.tasksOfDomain(board, id);
      const open = tasks.filter((task) => !TERMINAL_STATUS.has(task.status));
      const owner = this.expertOf(board, domain.ownerSessionId);
      domains.push({
        domainId: id,
        domainName: domain.name,
        responsibility: domainResponsibility(domain),
        notes: domain.notes,
        owner:
          owner === null
            ? null
            : {
                ...
                label: owner.label,
                role: owner.role,
                idle: !isAgentBusy(board, owner.sessionId, at),
              },
        openTasks: open.map((task) => ({ id: task.id, title: task.title, status: task.status })),
        doneCount: tasks.length - open.length,
        taskCount: tasks.length,
        lastActiveAt: tasks.reduce((latest, task) => Math.max(latest, task.endedAt || task.updatedAt || 0), 0),
      });
    }
    const experts = this.experts(board);
    return {
      request,
      domainId: wantedId,
      domains,
      // An expert whose domains are all gone still holds the context of the work
      // it did — worth seeing before opening a second agent in the same area.
      expertsWithoutDomains: experts.filter((expert) => expert.domains.length === 0),
      experts,
  ...
  /**
   * Declare a shared singleton this machine has one of (an environment, a
   * service, a build slot). The id is CALLER-CHOSEN and stored verbatim: the
   * plugin ships no default and never rewrites an id that is already on a
   * board, so a board written by an older revision (or by an operator with
   * different naming) keeps working — no migration, no alias table, no error.
   * Naming is a display concern: `label` is what the panel and the tools print.
   */
  defineResource(board, input) {
    const id = str(input.id).trim();
    if (id === "") throw new Error("资源需要 id（自定的短名，如 shared-env、build-slot、device-farm）");
   ...
   *
   * This exists because of the coordination model: a shared environment (the
   * singletons this machine has exactly one of) is granted BY the dispatcher,
   * never taken by a worker on its own. `force` is the handoff path — the
   * previous holder already returned its result and the dispatcher verified the
   * environment is free, so the lease moves in one recorded step instead of two.
   */
  grantResource(board, input) {
    const id = str(input.id).trim();
    if (id === "") throw new Error("grant 需要资源 id（如 shared-env、build-slot）");
    let resource = board.resources[id];
    if (resource === undefined) resource = this.defineResource(board, { id, label: str(input.label, id) });
    const sessionId = str(input.sessionId).trim();
